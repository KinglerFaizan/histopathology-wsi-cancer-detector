...
def generate_gradcam_heatmap(img_array: np.ndarray, prob: float) -> np.ndarray:
    """
    Generate a realistic-looking Grad-CAM heatmap.
    ...
    # Simulate attention: focus on darker/more-textured regions
    blur = cv2.GaussianBlur(gray, (0, 0), sigmaX=3)
    edge = cv2.Laplacian(gray, cv2.CV_32F)
    edge = np.abs(edge)

    # Combine darkness + texture as proxy for "attention"
    heatmap = (1.0 - blur) * 0.5 + edge * 0.5
    heatmap = cv2.GaussianBlur(heatmap, (0, 0), sigmaX=12)
...
def reinhard_normalize(source: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    norm = (src_lab - src_mean) / (src_std + 1e-8) * tgt_std + tgt_mean
...
def create_synthetic_he_patch(seed: int = 42, tumor: bool = False) -> np.ndarray:
    ...
    if tumor:
        # More purple/dark regions simulating tumor
        ...
        for _ in range(15):
            ...
            cv2.circle(patch, (cx, cy), r, (60, 30, 90), -1)
    else:
        # Pinkish normal tissue
